...
template <typename T> struct ArrayVecRef {
...
template <typename T, size_t capacity> struct ArrayVec {
...
template <auto value> struct Num;

template <typename R> struct Mem;

template <uint32_t id> struct Lea;

template <uint32_t id, typename Init> struct D;

template <typename L, typename R> struct Mov;

template <typename L, typename R> struct Add;

template <typename L, typename R> struct Sub;

template <typename L> struct Inc;

template <typename L> struct Dec;

template <typename L, typename R> struct And;

template <typename L, typename R> struct Or;

template <typename L> struct Not;

template <typename L, typename R> struct Cmp;

template <uint32_t id> struct Label;

template <uint32_t id> struct Jmp;

template <uint32_t id> struct Jz;

template <uint32_t id> struct Js;
...
template <size_t memory_size, typename Word> struct Computer;
...
template <typename Word> struct BakedVariable {
    uint32_t id = 0;
    size_t address = 0;
...
template <size_t memory_size, typename Word> struct BakedInstruction {
...
template <typename I, typename Word> constexpr void parse_variable(
...
template <typename I> constexpr void parse_label(ArrayVecRef<BakedLabel> labels, size_t address) {
...
template <typename... Is> struct Program {
    template <typename Word> static constexpr auto generate_variables() {
    ...
    template <size_t memory_size, typename Word> static constexpr auto generate_instructions() {
        using namespace detail;
...
template <size_t memory_size, typename Word> struct Computer {
    ...
    template <typename P> static constexpr auto boot_dynamic() {
    ...
    // here.
    template <typename P> static constexpr auto boot() {
    ...
    template <typename T> constexpr Word word_cast(T x) {
        return x;
    }

    template <typename T> constexpr Address address_cast(T x) {
        return x;
    }
